...
def iterative_value_matrix(gridworld_matrix, feed_list):
    ...
    next_gridworld_matrix = np.zeros((4, 4))
    for i in range(4):
        for j in range(4):
            if (i == 0 and j == 0) or (i == 3 and j == 3):  # the start and end
                continue
            for action in actions:
                ...
                next_gridworld_matrix[i][j] = next_gridworld_matrix[i][j] + action_select_prod[i][j][action] * (
                    action_reward[i][j][action] + gridworld_matrix[next_point_x, next_point_y])
